...
class UserLoginImpl(
    ...
    override fun execute(id: String, password: String): JwtResponse {
        ...
        val accessToken = jwtProvider.generateAccessToken(user.id!!, user.userTel, Role.ROLE_USER)
    ...
    private fun generateRefreshToken(user: User): String {
        ...
        if (refreshToken == null || !jwtProvider.isValidToken(refreshToken)) {
            refreshToken = jwtProvider.generateRefreshToken(user.id!!, user.username, Role.ROLE_USER)
